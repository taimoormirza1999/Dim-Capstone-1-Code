...
if (isset($_POST['update_profile'])) {
	...
	if (!$_FILES['profile_photo']['name']) {
		...
		$complete_image="../General_Assets/assets/img/" .$_SESSION['profile_img'];
		unlink($complete_image);
		// add new one
		$move_file = move_uploaded_file($c_img_tmp_name, $path);
	}
	$query = "UPDATE `admin_users` SET `name`='$p_name',`password`='$password',`profile_img`='$c_img_name' WHERE id=" . $_SESSION['user_id'];
...
$query_user = "SELECT id,name,profile_img,password FROM `admin_users` WHERE id=" . $_SESSION['user_id'];
			...
			<div class="outer-profile medium_b_raius mx-md-5" style="background:rgba(0,0,0,0.3);padding:1%;">
				<div class=" deep_dark p-md-5 p-3 text-light text-monospace px-md-5">
					<form method="post" action="<?php $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
								...
								<div class="col">
									<div class="row">
										<div class="sidebar-userpic">
											<img src="../General_Assets/assets/img/<?php echo $_SESSION['profile_img']; ?>" class="img-responsive" alt="" style="width:66px; margin:auto;" id="d_p_img">
										</div>
									</div>
								</div>
							</div>
						...
						<div class="crediet_card bg-light medium_b_raius p-3 pt-1 text-dark mx-md-5 shadow-lg">
							<div class="row">
								<div class="col">
